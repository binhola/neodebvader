...
class SimpleBlendDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(self.file_paths[file_idx], 'r') as f:
            # Get image (1, 45, 45)
            image = f['blend_images'][sample_idx][2,:,:]
...
dataset = SimpleBlendDataset(base_path=train_path, num_files=100)
...
def train_epoch(vae, device, dataloader, optimizer):
    ...
    for x, _ in dataloader:
        ...
        x_hat, kl = vae(x)
        loss, mse, kl = loss_function(x, x_hat, kl)
    ...
    return train_loss/n, train_mse/n, train_kl/n
...
def test_epoch(vae, device, dataloader):
    ...
    with torch.no_grad():
        for x, _ in dataloader:
            ...
            val_kl += kl
    n = len(dataloader.dataset)
    return val_loss / n, val_mse / n, val_kl / n

def save_checkpoint(epoch, vae, optimizer, train_loss, train_mse, train_kl, val_loss, val_mse, val_kl, checkpoint_path=f"checkpoints/vae_checkpoint_{name}.pth"):
    torch.save({
        'epoch': epoch,
